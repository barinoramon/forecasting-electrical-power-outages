...
class DataFormatter:
    ...
    def _convert_column_to_int(self, dataframe, column):
        #dataframe[column] = dataframe[column].astype(str).replace(',', '.', regex=False)
        dataframe[column] = dataframe[column].apply(lambda x: float(str(x).replace(',', '.')))
        dataframe[column] = dataframe[column].fillna(0).astype(float).round().astype(int)
        return dataframe[column]
